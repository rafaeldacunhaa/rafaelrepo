...
export class PiPController {
    ...
    private pipStatusIndicator: HTMLElement | null;
    private autoPiPEnabled: boolean = true;
    private isPageVisible: boolean = true;
    private autoPiPCheckInterval: number | null = null;
    ...
    private startAutoPiPCheck(): void {
    ...
    private checkAndOpenPiPIfNeeded(): void {
    ...
    private async openPiPAutomatically(): Promise<void> {
        try {
    ...
    private isTimerRunning(): boolean {
        ...
        if (!pauseButton || !pauseButton.textContent) return false;
        
        const isRunning = pauseButton.textContent.includes('Pausar');
        return isRunning;
    }

    private showAutoPiPNotification(): void {
    ...
    public async toggleAutoPiP(): Promise<void> {
    ...
    private updateAutoPiPButton(): void {
        if (this.pipButton) {
    ...
    private createStatusIndicator(): void {
        ...
        // Estilos para o dot
        const dot = this.pipStatusIndicator.querySelector('.pip-status-dot') as HTMLElement;
        if (dot) {
            dot.style.cssText = `
                width: 100%;
                height: 100%;
    ...
    private setupEventListeners(): void {
        ...
        document.addEventListener('pipVisibilityChanged', ((event: Event) => {
            const customEvent = event as CustomEvent;
            this.handlePiPVisibilityChanged(customEvent.detail);
        }) as EventListener);
    }

    private handlePiPVisibilityChanged(detail: any): void {
    ...
    public async restorePiP(): Promise<void> {
        if (this.pipManager.isOpen()) {
    ...
    private updatePiPButton(isOpen: boolean, isVisible: boolean, hasLostFocus: boolean): void {
        if (this.pipButton) {
