...
export class BlocoManager {
    ...
    public findFirstUnfinishedBlocoIndex(): number {
        const unfinishedIndex = this.blocos.findIndex(bloco => !bloco.isDone);
        return unfinishedIndex !== -1 ? unfinishedIndex : -1;
    }

    public findNextUnfinishedBlocoIndex(currentIndex: number): number {
        if (this.blocos.length === 0) return -1;
    ...
    public findPrevUnfinishedBlocoIndex(currentIndex: number): number {
        if (this.blocos.length === 0) return -1;
    ...
    public toggleBlocoDone(id: string): void {
        const bloco = this.blocos.find(b => b.id === id);
        if (bloco) {
    ...
    public markBlocoAsDone(id: string): void {
        const bloco = this.blocos.find(b => b.id === id);
        if (bloco) {
            bloco.isDone = true;
            this.saveToStorage();
    ...
    public markBlocoAsNotDone(id: string): void {
        const bloco = this.blocos.find(b => b.id === id);
        if (bloco) {
            bloco.isDone = false;
            this.saveToStorage();
